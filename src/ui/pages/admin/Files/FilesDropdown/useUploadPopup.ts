...
export const useUploadPopup = () => {
  ...
  const postFile = async (fileItems: (FileItem | null)[]) => {
    ...
    try {
      ...
      if(result[0]?._id) {
        adminLogManager?.createAddLog({
          screen: AdminLogScreen.PDFFiles,
          target: {
            type: AdminLogTargetType.PDFFiles,
            // _id: this.form._id,
            _id: result[0]?._id || null,
            origin: result[0]?.name || null,
            path: result[0]?.path || null,
            message: 'PDF files added',
            _key: 'ap'
          },
          resource: {
            // ...this.form,
  ...
  const updateFile = async (fileItems: (FileItem | null)[], fileId: string) => {
    ...
    try {
      ...
      console.log("useUploadPopup::updateFile: ", result);
